<script lang="ts">
	...
	const modalComponentExercise: ModalComponent = {
		...
		props: { exercise_options: data.exercise_choices, data: data.exerciseForm, post_link: `/edit_exercise_routine/${data.exerciseForm?.data.id}` }
	};
